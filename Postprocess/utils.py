...
def plotMetricSurf(X_2d, met_2d, metrics, metLab, ncols=4, nPts=20, thr=0, 
                   ...
                   fac=3, lw=0.2, surf=True, reject_outliers=True):                    

    if reject_outliers:
        sh0 = X_2d.shape[0]
        X_2d,rows = rejectOutliers(X_2d)
        rows_keep = np.ones(sh0, dtype=bool)
        rows_keep[rows] = False
        met_2d = met_2d[rows_keep]
